!(function(){
    ...
    Generator._generator = function( name, schema, wholeScheme, options ){
        ...
        if( schema.$ref ){
            ...
            // 查找配置中是否给定了
            else if( /^[^\/]+\/[^\/]+$/.test( schema.$ref ) ){
                external = true;
                externalURL = 'http://dip.alibaba-inc.com/api/v2/schemas/' + encodeURIComponent( schema.$ref ) + '/content';
                if( typeof options.namespaceRemoteURL == 'function' ){
                    externalURL = options.namespaceRemoteURL( schema.$ref );
                }
                var externalSchema = null;
                // 简单clone下
                if( options.refs && options.refs[ schema.$ref ] ) {
                    externalSchema = JSON.parse( JSON.stringify( options.refs[ schema.$ref ]) );
                } else {
                    externalSchema = {};
                }
