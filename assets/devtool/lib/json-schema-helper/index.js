!(function(){
    ...
    var Mod = {
         ...
         * @returns {boolean}
         */
        ifCustomKeyword: function( path ){
            var currentKey = this.getCurrentKey( path );
            return this.ifKeyword( path ) && currentKey.indexOf( 'x-' ) == 0;
        },
        ...
        _ifKeyword: function( key, parentKey, parentKeyKeyword ){
            ...
            if( !parentKey ){
                // 真正关键词或者自定义关键词
                return allKeywords.indexOf( key ) >= 0 || key.indexOf( 'x-' ) == 0;
            }

            else {
                if( allKeywords.indexOf( key ) >= 0 || key.indexOf( 'x-' ) == 0 ) {
                    // 若夫路径为关键词
                    if( parentKeyKeyword ){
